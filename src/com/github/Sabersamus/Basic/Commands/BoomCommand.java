...
public boolean onCommand(CommandSender cs, Command cmd, String aliases, String[] args) {
	if(cmd.getName().equalsIgnoreCase("boom") && cs.hasPermission("basic.boom") && cs instanceof Player){
		...
		if(args.length == 0){
		((Player)player).getWorld().createExplosion(player.getLocation(), 10);
		...
		if(cs instanceof Player){
			...
			cs.sendMessage(ChatColor.DARK_GREEN + "LOLZ you just blew up " + target.getDisplayName());
			Bukkit.broadcastMessage(ChatColor.DARK_PURPLE + target.getDisplayName() + ChatColor.DARK_PURPLE + " JUST EXPLODED!");
			return true;
		}
	}else{
		if(args.length > 1){
			cs.sendMessage(ChatColor.RED + "What exactly are you trying to do?");
		}
	}
	}
	return false;
}else{
	cs.sendMessage(ChatColor.RED  + "You are not allowed to use this command");
}
	return false;
}
}